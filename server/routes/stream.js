...
function proxyR2(key, contentType, cacheSeconds, req, res) {
...
router.get("/thumbnails/:filename", (req, res) => {
  ...
  proxyR2(`thumbnails/${filename}`, mime, 86400, req, res);
});
...
router.get("/previews/:videoId", (req, res) => {
  ...
  proxyR2(`previews/${videoId}.mp4`, "video/mp4", 3600, req, res);
});
...
router.get("/fallback/:videoId", (req, res) => {
  const { videoId } = req.params;
  if (!/^[a-zA-Z0-9_-]+$/.test(videoId)) return res.status(400).end();
  proxyR2(`fallback/${videoId}.mp4`, "video/mp4", 86400, req, res);
});
...
router.get("/:videoId/:filename", (req, res) => {
  ...
  proxyR2(`videos/${videoId}/${filename}`, mime, 3600, req, res);
});
